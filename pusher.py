    ...
    while not TERMINATE:
        ...
        if path.exists("debug"):
            if not debug: #If debug file was just created, enable DEBUG
                debug = True
                logging.basicConfig(filename="pusher.log", format=FORMAT, level=logging.DEBUG, force=True)
                logging.info("DEBUG logging ENABLED")
        else:
            if debug: #If debug file was just deleted disable it (regardless of --debug start)
                debug = False
                logging.basicConfig(filename="pusher.log", format=FORMAT, level=logging.INFO, force=True)
